...
// From boost/dynamic_bitset/config.hpp
...
public:
    ...
    // block_width_type
    enum { bits_per_block_const = (std::numeric_limits<Block>::digits) };
    inline static block_width_type bits_per_block()
      { return static_cast < block_width_type >(bits_per_block_const); }
    // size_type
    enum { npos_const = -1 };
    inline static size_type npos()
      { return static_cast < size_type >(npos_const); }
...
private:
    // block_width_type
    enum { ulong_width = std::numeric_limits<unsigned long>::digits };
...
dynamic_bitset<Block, Allocator>::count() const
{
    ...
    const bool no_padding = bits_per_block_const == CHAR_BIT * sizeof(Block);
